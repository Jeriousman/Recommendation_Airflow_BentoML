...
with DAG(
    ...
    schedule_interval="30 * * * *", #*/30 * * * *",  ##meaning every 2 hours. https://crontab.guru/ 매새벽3시에돌린다 (UTC타임)
    ...
    task_create_bento = BashOperator(   ##이전 bentoml model들을 모두 지운다
        ...
        """ 
            ...
            cd /opt/airflow/dags/bentoml/user_rec_bento; bentoml build  
        """
        )
    ...
    task_serve_bentoml = BashOperator(
        ...
        # fuser -k 3000/tcp; 
        """ 
           fuser -k 3000/tcp; bentoml serve -p 3000 user_recommender_bento:latest --production
        """
        )
