...
runner = bentoml.transformers.get("user_to_pik_recommender_model:latest").to_runner()

# runner.init_local()
# svc = bentoml.Service("feature-extraction-service", runners=[runner])
svc = bentoml.Service("user_to_pik_recommender_bento", runners=[runner])
...
def get_most_similar_users_for_pik_ko(pik_id, user_id, user_vec, pik_vec, friends_list, user_lang_dict,  num_link_by_user, topk, threshold, num_link_threshold):
    ...
    for uid, vec in user_vec.items():
        if user_lang_dict[uid] == 'ko' or user_lang_dict[uid] == 'kr':
            ...
            thisSim = cosine_similarity(np.array(vec).reshape(1, -1), np.array(pik_vec[pik_id]).reshape(1, -1))
            sim.append((uid, thisSim[0][0]))
            
    not_me_not_friends_rec = [] ## filtering
    for (uid, similarity) in sim:
        if user_id != uid and int(uid) not in list(friends_list['followed_user_id'][friends_list['user_id'] == int(user_id)]):
            not_me_not_friends_rec.append((uid, similarity))
            
    if not not_me_not_friends_rec: ##if not_me_not_friends_rec is empty list,
        pass
        # return not_me_not_friends_rec
    else:
        full_ranked_similar_items = sorted(not_me_not_friends_rec, key=lambda x: x[1], reverse=True) ##full similarity list
        ranked_similar_items = full_ranked_similar_items[:topk+1] ##only topk similarity list. 본픽도 들어가있기떄문에+1을해준다
                
        '''
        현재 보고 있는 픽의 주인 자기 자신이 추천되면 안됨
        현재 보고 있는 픽의 주인이 이미 친구인 유저를 추천하면 안됨
        추천 후보 픽에 이미 올라온 추천 후보를 중복해서 다시 올리면 안됨
        '''
        
        sim_list = [] ##추천 candidate 추려내서 저장하는 리스트 
        for i in range(0, topk+1):
            if ranked_similar_items[i][1] > threshold:
                if ranked_similar_items[i][0] not in list([sim_list[num]['user_id'] for num in range(len(sim_list))]): ##본 유저가 아니거나 본 유저가 팔로잉 있지 않은 유저면 추천하라는 것 
                    if i >= 1:  
                        if ranked_similar_items[i-1][1] != ranked_similar_items[i][1]: ##유사도가 바로그다음으로높은것과비교했을때 현재유사도와같으면 같은내용의픽이나링크일테니 그건스킵하라는것
                            if num_link_by_user[ranked_similar_items[i][0]] >= num_link_threshold: ##픽안에 num_link_threshold 갯수이상 링크가 존재할때만 추천한다 
                                lottery = random()
...
# df['language_code'][df['user_id']==6905]
    
def get_most_similar_users_for_pik_en(pik_id, user_id, user_vec, pik_vec, friends_list, user_lang_dict,  num_link_by_user, topk, threshold, num_link_threshold):
...
def rec_users_for_pik_by_lang(pik_id, user_id, pik_vec, user_vec, friends_list, user_lang_dict, pik_lang_dict, num_link_by_pik, num_link_by_user, topk, threshold, num_link_threshold):  
    if pik_id in pik_lang_dict.keys():
    
        if pik_lang_dict[pik_id] == 'ko' or pik_lang_dict[pik_id] == 'kr':
            if pik_id in num_link_by_pik.keys():
                result = get_most_similar_users_for_pik_ko(pik_id, user_id, user_vec, pik_vec, friends_list, user_lang_dict,  num_link_by_user, topk, threshold, num_link_threshold)
                return result
            
            elif pik_id not in num_link_by_pik.keys():
                print('이 픽은 한국어를 사용하고 업데이트 됐으나 픽 안의 링크가 없거나 링크 업데이트가 안되었다')
                sim_list = list()
                while True: 
        ...
        elif pik_lang_dict[pik_id] == 'en':
            if pik_id in num_link_by_pik.keys():   
                result = get_most_similar_users_for_pik_en(pik_id, user_id, user_vec, pik_vec, friends_list, user_lang_dict,  num_link_by_user, topk, threshold, num_link_threshold)
                return result
    ...
    elif pik_id not in pik_lang_dict.keys():   ##만약 이 픽이 아직 업데이트 안된 신규 픽이라면
        print('픽언어는 관심없고 픽등록도 안되었거나 업데이트가 안되었으면 암거나 추천해준다')
        sim_list = list()
        while True: 
            key, value = choice(list(num_link_by_user.items()))
...
def predict(user_id, pik_id) -> dict:
    ...
    similarity_dict = rec_users_for_pik_by_lang(pik_id, user_id, pik_vec, user_vec, user_friends_list, user_lang_dict, pik_lang_dict, num_link_by_pik, num_link_by_user, topk=30, threshold=0.7, num_link_threshold=4)
