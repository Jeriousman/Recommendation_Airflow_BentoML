...
# distilbert_runner = bentoml.transformers.get("link_recommender_model:latest").to_runner()
# sentence_transformers_runner = bentoml.picklable_model.get("sentence_transformer_model:latest").to_runner()
...
class link_rec_runnable(bentoml.Runnable):
    SUPPORTED_RESOURCES = ("nvidia.com/gpu",)
    SUPPORTS_CPU_MULTI_THREADING = True

    def __init__(self):
        self.fasttext_model = fasttext.load_model('/opt/airflow/dags/data/lid.176.bin')
        self.sentence_transformer_model = SentenceTransformer('sentence-transformers/distilbert-multilingual-nli-stsb-quora-ranking')

        # self.lsh = lshashpy3.LSHash(hash_size=60, input_dim=768, num_hashtables=60,
    ...
    def predict_emb(self, processed_text):
        return self.model.encode(processed_text)


    @bentoml.Runnable.method(batchable=True, batch_dim=0)
    def predict_lang(self, processed_text):
        return self.model.predict(processed_text)[0][0][0][-2:]


    # @bentoml.Runnable.method(batchable=True, batch_dim=0)
    # def recommend(self, processed_text):
    #     return self.model.predict(processed_text)[0][0][0][-2:]




link_rec_runner = bentoml.Runner(link_rec_runnable, max_batch_size=30)
...
svc = bentoml.Service("link_recommender_bento", runners=[link_rec_runner])
# svc = bentoml.Service("link_recommender_bento", runners=[distilbert_runner, sentence_transformers_runner, fasttext_runner])
...
async def language_predict(link_text) -> dict:   
    language = await link_rec_runner.predict_lang.async_run([link_text])
    # similarity_list = rec_link_by_lang(link_id, user_id, link_text, link_status_dict, link_lang_dict_detected, user_lang_dict_detected, lsh, lsh_topk=50, final_top_k=5, threshold=0.3)
...
# input_spec = Multipart(user_id=Text(), link_id=Text(), link_text=Text())
@svc.api(input=Text(), output=JSON())
async def embedding_predict(link_text) -> dict:   
    embedding = await link_rec_runner.predict_emb.async_run([link_text])
    # similarity_list = rec_link_by_lang(link_id, user_id, link_text, link_status_dict, link_lang_dict_detected, user_lang_dict_detected, lsh, lsh_topk=50, final_top_k=5, threshold=0.3)
    # similarity_dict = {similarity_pair[0]: similarity_pair[1] for similarity_pair in similarity_list} ##dict {user: similarity}   
    return embedding
...
# def predict(user_id, link_id, link_text) -> dict:   
#     similarity_list = rec_link_by_lang(link_id, user_id, link_text, link_status_dict, link_lang_dict_detected, user_lang_dict_detected, lsh, lsh_topk=50, final_top_k=5, threshold=0.3)
#     similarity_dict = {similarity_pair[0]: similarity_pair[1] for similarity_pair in similarity_list} ##dict {user: similarity}   
