...
with open("/home/hojun/airflow/dags/data/user_vec.json") as f:  ##avg_{data_type}_vec.pickle  avg_pik_vec.json or avg_user_vec.json
...
user_friend_list = pd.read_csv("/home/hojun/airflow/dags/data/users_following.csv")
...
num_link_threshold=0.7
friends_list=user_friend_list
user_friend_list.dtypes
...
def get_most_similar_users_ko(user_id, user_vec, friends_list, num_link_by_user, topk, threshold, second_threshold, num_link_threshold):
    ...
    for i in range(1, topk+1):
        if ranked_similar_items[i][1] > threshold:
            if user_id != ranked_similar_items[1][0] and int(ranked_similar_items[1][0]) not in list(friends_list['followed_user_id'][friends_list['user_id'] == int(user_id)]): ##본 유저가 아니거나 본 유저가 팔로잉 있지 않은 유저면 추천하라는 것
                if i >= 1:  
                    if ranked_similar_items[i-1][1] != ranked_similar_items[i][1]: ##유사도가 바로그다음으로높은것과비교했을때 현재유사도와같으면 같은내용의픽이나링크일테니 그건스킵하라는것
                        if num_link_by_user[ranked_similar_items[i][0]] >= num_link_threshold: ##픽안에 num_link_threshold 갯수이상 링크가 존재할때만 추천한다 
                            sim_list.append((ranked_similar_items[i][0], ranked_similar_items[i][1]))
