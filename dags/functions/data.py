...
def raw_data_preprocess(**kwargs):
    ...
    ### hashtaghub = pd.read_csv(f'{path}/hashtaghub_tag.csv')
    ...
    user_language.rename(columns = {'id': 'user_id'}, inplace=True)
    ...
    linkhub.rename(columns = {'title':'link_title', 'category_id':'cat_id', 'id':'link_id', 'created':'link_created', 'description':'link_description','memo':'link_memo', 'is_draft':'link_is_draft','is_deleted':'link_is_deleted','platform':'link_platform'}, inplace=True)
    linkhub = linkhub[(linkhub['link_is_deleted'] == 'f') | (linkhub['link_is_deleted'] == False)]
    linkhub.drop(columns = ['modified','url','picture_id','default_logo','position','search_vector','old_id','curating_point','next_obj','previous_obj','imported' ], inplace=True)   
    
    ### taghub.rename(columns = {'id':'hashtag_id'}, inplace=True)
    
    
    piks.rename(columns = {'title':'pik_title', 'id':'pik_id','is_default':'pik_is_default','is_draft':'pik_is_draft','slug':'pik_slug','memo':'pik_memo'}, inplace=True)
    piks = piks[(piks['is_deleted'] == 'f') | (piks['is_deleted'] == False)]
    
    
    catego.rename(columns = {'title': 'cat_title','is_default':'cat_is_default', 'id':'cat_id'}, inplace = True)
    catego = catego[(catego['is_deleted'] == 'f') | (catego['is_deleted'] == False)]
    catego.drop(columns = ['created','modified','position','is_initial','old_id','is_deleted','platform','next_obj','next_obj','previous_obj'],inplace=True)
    ...
    catego = pd.merge(catego, user_language[['user_id', 'language_code']], how = 'inner', on='user_id', suffixes=('', '_user'))



    ##pik_info와 cat_info 병합
    piks_cats = pd.merge(catego, piks[['pik_id', 'pik_title', 'pik_slug', 'status', 'pik_is_default','pik_is_draft']], how = 'inner', left_on = 'pik_id', right_on = 'pik_id', suffixes=('_cat', '_pik'))
    piks_cats.columns
    # filtered_pik_cat  = piks_cats[(piks_cats['status'] == 'public') & (piks_cats['is_draft'] == 'f')] #'f' instead of False
    public_private_filtered_pik_cat = piks_cats[(piks_cats['pik_is_draft'] == 'f') | (piks_cats['pik_is_draft'] == False)]
    ...
    link_cat_pik = pd.merge(linkhub, public_private_filtered_pik_cat, how='inner', on='cat_id', suffixes=('', '_catpik'))
    link_cat_pik.sort_values(['user_id', 'cat_id', 'link_id'], ascending = True, inplace=True)  
    ...
    pik_status_dict = {k:v for k,v in zip(link_cat_pik['pik_id'], link_cat_pik['status'])}
    link_status_dict = {k:v for k,v in zip(link_cat_pik['link_id'], link_cat_pik['status'])}
    
    
    linkid_title_dict = {k:v for k,v in zip(link_cat_pik['link_id'], link_cat_pik['link_title'])}
    pikid_title_dict = {k:v for k,v in zip(link_cat_pik['pik_id'], link_cat_pik['pik_title'])}
    ...
    #         slug1 = data['pik_slug'][data['pik_id'] == 3085].iloc[0]
    #         slug2 = data['pik_slug'][data['pik_id'] == 3186].iloc[0]
    #         slug3 = data['pik_slug'][data['pik_id'] == 3188].iloc[0]
            
    #         ##특정 string이 포함된 행을 df에서찾기
    #         dummy1 = data[data['pik_slug'].str.contains(slug1)]
    #         dummy2 = data[data['pik_slug'].str.contains(slug2)]
    #         dummy3 = data[data['pik_slug'].str.contains(slug3)]
    ...
    #         ##제거된것을확인
    #         data[data['pik_slug'] == slug1]
    #         data[data['pik_slug'] == slug2]
    #         data[data['pik_slug'] == slug3]
            
    #         # data.drop(['memo', 'url', 'is_draft_link', 'link_create_time', 'id_cat', 'created_cat', 'id_user', 'id_pik', 'slug', 'language', 'is_draft_catpik', 'created_pik'], axis=1, inplace=True)
            
    #     if integering:
    #         data = data.astype({'pik_id' :'int', 'link_id':'int', 'cat_id':'int'})    
    ...
    #     user_lang_dict_userset = {str(k):str(v) for k,v in zip(user_language_df['user_id'], user_language_df['language_code'])} ##모든유저를 검색할 수 있도록
    ...
    #     return user_lang_dict_userset, pik_lang_dict_userset, link_lang_dict_userset, pik_status_dict, link_status_dict, linkid_title_dict, pikid_title_dict, data
    ...
    # user_lang_dict_userset, pik_lang_dict_userset, link_lang_dict_userset, pik_status_dict, link_status_dict, linkid_title_dict, pikid_title_dict, link_cat_pik = dropNa_dropArtificialUser(link_cat_pik, artificial_users, user_language, dropna=True, drop_artificial=True, integering=True)
    ...
    with open(f"{path}/link_status_dict.json", "w") as f:   ##For bento_service.py
        json.dump(link_status_dict, f)
